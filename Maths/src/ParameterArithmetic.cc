...
static const double minLL   = -1/(epsilon*epsilon);
static const double mindiv  = -1/(epsilon*epsilon) ;
static const double maxdiv  = -mindiv ;
static const size_t max_iteration = 1e+4;

static const double minerr     = 1e-4;
...
IterateSolver( gsl_multiroot_fsolver* solver )
{
   ...
   do {
      ...
      //  printf( "%20.10f " , gsl_vector_get(solver->x, i) );
      // } printf("|") ;
      // for( size_t i  = 0 ; i < solver->x->size ; ++i ){
      //  printf( "%20.10f " , gsl_vector_get(solver->f, i) );
      // } printf("\n");
...
Prod( const vector<Parameter>& parmlist )
{
   ...
   *******************************************************************************/
   ...
   for( size_t i = 0; i < parmlist.size(); ++i  ){
      ...
      const double hatx  = 1;
      const double xup   = 1 + (p.RelUpperError() + minerr)/1.2;
      const double xdown = 1 - (p.RelLowerError() + minerr)/1.2;
